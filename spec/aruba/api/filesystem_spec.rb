...
RSpec.describe Aruba::Api::Filesystem, type: :aruba do
  include_context 'uses aruba API'

  let(:name) { @file_name }
  let(:path) { @file_path }
  let(:dir_name) { 'test.d' }
  let(:dir_path) { expand_path(dir_name) }
  ...
  describe '#touch' do
    context 'when touching a file that does not exist' do
      ...
      it 'creates an empty file in a non-existing directory' do
        name = 'directory/test'
        path = File.join(aruba.current_directory, name)
      ...
      it 'creates a file relative to home if name includes ~' do
        ...
        path = File.join(aruba.home_directory, string)
      ...
      it 'creates multiple files when multiple names are given' do
        ...
        paths = names.map { |name| File.join(aruba.current_directory, name) }
        aggregate_failures do
          paths.each do |path|
            expect(File.file?(path)).to be true
          end
  ...
  describe '#absolute?' do
    it 'returns true when argument is an absolute path' do
      expect(absolute?(path)).to be true
    end

    it 'returns false when argument is a relative path' do
      expect(absolute?(name)).to be false
    end
  end

  describe '#relative?' do
    it 'returns false when given an absolute path' do
      expect(relative?(path)).to be false
    end

    it 'returns true when given a relative path' do
      expect(relative?(name)).to be true
    end
  end
  ...
  describe '#exist?' do
    context 'when given a file' do
      it 'returns true if the file exists' do
        ...
        expect(exist?(name)).to be true
      end

      it 'returns false if the file does not exist' do
        expect(exist?(name)).to be false
      end
    end
    ...
    context 'when given a directory' do
      it 'returns true if the directory exists' do
        ...
        expect(exist?(dir_name)).to be true
      end

      it 'returns false if the directory does not exist' do
        expect(exist?(dir_name)).to be false
      end
    end
  end
  ...
  describe '#file?' do
    context 'when given a file' do
      context 'when it exists' do
        ...
        it { expect(file?(name)).to be true }
      end

      context 'when does not exist' do
        it { expect(file?(name)).to be false }
      end
    end
  ...
  describe '#directory?' do
    context 'when given a file' do
      context 'when it exists' do
        ...
        it { expect(directory?(name)).to be false }
      end
    ...
    context 'when given a directory' do
      context 'when it exists' do
        ...
        it { expect(directory?(dir_name)).to be true }
      end

      context 'when does not exist' do
        it { expect(directory?(dir_name)).to be false }
      end
    end
  end
    ...
    context 'when source is existing' do
      context 'when destination is non-existing' do
        ...
        context 'when source is directory' do
          ...
          before do
            Aruba.platform.mkdir(File.join(aruba.current_directory, source))
            copy source, destination
          end
      ...
      context 'when destination is existing' do
        context 'when source is list of files' do
          ...
          context 'when destination is directory' do
            ...
            before do
              Aruba.platform.mkdir(File.join(aruba.current_directory, destination))
              copy source, destination
            end
          ...
          context 'when destination is not a directory' do
            ...
            it 'raises an appropriate error' do
              expect { copy source, destination }
          ...
          context 'when a source is the same like destination' do
            ...
            it 'raises an appropriate error' do
              src_path = File.expand_path(File.join(aruba.current_directory, source))
              dest_path = File.expand_path(File.join(aruba.current_directory,
                                                     destination))
      ...
      context 'when source is non-existing' do
        it { expect { copy source, destination }.to raise_error ArgumentError }
      end
    end
  end
  ...
  describe '#write_fixed_size_file' do
    ...
    it 'writes a fixed sized file' do
      write_fixed_size_file(name, file_size)
      expect(File.exist?(path)).to be true
    ...
    it 'works with ~ in path name' do
      ...
      with_environment 'HOME' => File.expand_path(aruba.current_directory) do
        write_fixed_size_file(file_path, file_size)
  ...
  describe '#chmod' do
    ...
    context 'when file exists' do
      ...
      context 'and path has ~ in it' do
        ...
        let(:path) { File.join(aruba.current_directory, basename) }
  ...
  describe '#with_file_content' do
    ...
    it 'works with ~ in path name' do
      ...
      with_environment 'HOME' => File.expand_path(aruba.current_directory) do
        write_file(file_path, 'foo bar baz')

        with_file_content file_path do |full_content|
          expect(full_content).to eq 'foo bar baz'
        end
      end
    end
  ...
  describe '#read' do
    ...
    context 'when does not exist' do
      it { expect { read(name) }.to raise_error ArgumentError }
    end
    ...
    context 'when it exists' do
      context 'when file' do
        ...
        context 'when normal file' do
          it { expect(read(name)).to eq [content] }
        end
  ...
  describe '#list' do
    ...
    context 'when does not exist' do
      it { expect { list(name) }.to raise_error ArgumentError }
    end
  ...
  describe '#remove' do
    ...
    context 'when given an item that does not exist' do
      it 'raises an error' do
        name = 'missing'
        expect { remove(name) }.to raise_error Errno::ENOENT
      end
      ...
      it 'raises no error when forced to delete the file' do
        ...
        aggregate_failures do
          expect { remove(name, force: true) }.not_to raise_error
          expect(File.exist?(path)).to be false
        end
      end
    end
