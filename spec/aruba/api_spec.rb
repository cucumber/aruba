...
describe Aruba::Api do
      ...
      context "#check_file_content" do
        context "with regexp" do
          ...
          it "succeeds if file content matches" do
            expect(@file_name).to have_file_content(matching_content)
            expect(@file_name).to have_file_content(matching_content, true)
          end

          it "succeeds if file content does not match" do
            expect(@file_name).to_not have_file_content(non_matching_content)
          end
          ...
          it "works with ~ in path name" do
            ...
            @aruba.with_environment 'HOME' => File.expand_path(aruba.current_directory) do
              ...
              expect(file_path).to_not have_file_content(non_matching_content)
            end
          end
        end
    ...
    describe '@announce_stdout' do
      ...
      context 'enabled' do
        ...
        it "should announce to stdout exactly once" do
          ...
          expect(@aruba.last_command_started.stdout).to include('hello world')
        end
      end
  ...
  describe "#set_environment_variable" do
    ...
    it "set environment variable" do
      ...
      wait_for { @aruba.last_command_started.stdout =~ /LONG_LONG/ }
      expect(@aruba.last_command_started.stdout).to include("LONG_LONG_ENV_VARIABLE=true")
    end
    ...
    it "overwrites environment variable" do
      ...
      @aruba.run_simple "env"
      wait_for { @aruba.last_command_started.stdout =~ /LONG_LONG/ }
      expect(@aruba.last_command_started.stdout).to include("LONG_LONG_ENV_VARIABLE=false")
    end
  end
end # Aruba::Api
