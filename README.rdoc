...
* The description from the scenario (You can use textile here. Escape * and # at beginning of line with \)
...
Reporting is off by default, but you can enable it by defining a special environment variable where reports are written:

    ARUBA_REPORT_DIR=doc cucumber features

The generated HTML pages are meant to be included in other pages, so they do not contain html or body tags, or any css.
This gives you the liberty to embed the HTML in other pages and style them as you like.

TODO: Run description through Markdown (RDiscount)
TODO: post-process the files and put it all in one HTML file.
TODO: add a mechanism to add a layout template
